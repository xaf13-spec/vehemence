...
export default function GameCard({ game, favorite = false, onFavorite, onOpen }) {
  ...
  function startGame() {
    ...
    localStorage.setItem("vehemence_recent_games", JSON.stringify([game.name, ...recent].slice(0, 8)));
  ...
  return (
    <>
      <article className="game-card">
        ...
        <button type="button" className={`game-favorite ${favorite ? "active" : ""}`} aria-label={favorite ? `Remove ${game.name} from favorites` : `Add ${game.name} to favorites`} onClick={() => onFavorite?.(game.name)}>
          {favorite ? "★" : "☆"}
        </button>
      </article>

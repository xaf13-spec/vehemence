...
export default function PersistentSpotifyPlayer() {
  ...
  useEffect(() => {
    const select = (event) => loadSelection(event.detail);
    ...
    const createController = (IFrameAPI) => {
      ...
      IFrameAPI.createController(
        ...
        (controller) => {
          ...
          if (pending) {
            controller.loadEntity(`spotify:${pending.type || "playlist"}:${pending.id}`);
            setCurrent(pending);
            setVisible(true);
          }
          ...
          controller.addListener(IFrameAPI.EVENTS.PLAYBACK_UPDATE, (event) => {
            ...
            if (position < 1000) {
              ...
              lastRestartRef.current = 0;
            } else if (track && track !== currentTrackRef.current) {

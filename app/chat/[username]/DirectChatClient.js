...
export default function DirectChatClient({ username, currentUserId, embedded = false, onBack }) {
  ...
  async function handleSubmit(event) {
    event.preventDefault(); const text = message.trim(); if (!text || sending) return; setSending(true); setError("");
    try {
      const response = await fetch("/api/direct-chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username, message: text }) }); const data = await response.json();
      if (!response.ok) { setError(data.error || "Could not send the message."); return; }
      if (data.message) setMessages((current) => current.some((item) => item.id === data.message.id) ? current : [...current, data.message]); setMessage("");
    } catch { setError("Could not send the message."); } finally { setSending(false); }
  }

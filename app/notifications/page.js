...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("vehemence_notifications");
    if (saved) setNotifications(JSON.parse(saved));
  }, []);

  function persist(next) {
    setNotifications(next);
    localStorage.setItem("vehemence_notifications", JSON.stringify(next));
  }
    ...
    <main className="notifications-page">
      <div className="notifications-container">
        ...
        <section className="notifications-card">
          {notifications.length ? notifications.map((item) => <article className={`notification-item ${item.read ? "read" : ""}`} key={item.id}><span className="notification-dot" /><div><div className="notification-top"><h2>{item.title}</h2><span>{item.type}</span></div><p>{item.text}</p></div></article>) : <div className="notification-empty"><h2>You're all caught up</h2><p>New notifications will appear here.</p></div>}
        </section>
      </div>
    </main>
  );
